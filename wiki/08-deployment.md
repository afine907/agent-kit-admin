...
if [ ! -f deploy/.env ]; then
    ...
    $SED_CMD "s/change_me_to_strong_password/$DB_PASSWORD/" deploy/.env
    $SED_CMD "s/change_me_to_strong_password/$MINIO_PASSWORD/" deploy/.env
    $SED_CMD "s/change_me_to_random_string_at_least_32_chars/$JWT_SECRET/" deploy/.env

    echo "✅ .env file created with random passwords"
    echo "⚠️  Please edit deploy/.env to configure OAuth providers"
fi
...
echo ""
echo "🌐 Web UI: https://$(grep DOMAIN deploy/.env | cut -d= -f2)"
echo "📡 API: https://$(grep DOMAIN deploy/.env | cut -d= -f2)/api/v1"
echo ""
echo "📝 Next steps:"
echo "   1. Edit deploy/.env to configure OAuth providers"
