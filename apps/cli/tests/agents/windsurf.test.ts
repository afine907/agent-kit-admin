...
import os from 'os';
import { WindsurfAdapter } from '../../src/agents/windsurf';
...
describe('WindsurfAdapter', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'akit-test-windsurf-'));
    adapter = new WindsurfAdapter(tempDir);
    configPath = path.join(tempDir, '.windsurf', 'mcp.json');
  });
  ...
  it('should return correct config path', () => {
    ...
    expect(cfgPath).toContain('.windsurf');
    expect(cfgPath).toContain('mcp.json');
    expect(cfgPath).toBe(configPath);
  });

  it('should detect Windsurf installation', async () => {
