...
> **说明：** Worker 进程与 FastAPI Server 共享代码库、数据库和 MinIO，但作为独立进程运行。它通过 APScheduler 定时执行健康检测，发布/版本更新时通过 DB `needs_check` 标记触发增量检测。详见 `docs/architecture/21-inspector-worker.md`。
