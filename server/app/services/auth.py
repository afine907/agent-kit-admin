...
class AuthService:
    ...
    def _cleanup_expired_states(self) -> None:
        ...
        expired_keys = [
            key for key, value in _oauth_state_store.items()
            if value["expires"] < now
        ]
        for key in expired_keys:
            del _oauth_state_store[key]

    def _verify_oauth_state(self, state: str, provider: str) -> None:
    ...
    async def handle_oauth_callback(self, provider: str, code: str, state: str = None) -> dict:
